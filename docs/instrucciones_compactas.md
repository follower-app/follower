...
README · REGLAS_IA · docs/: contexto_maestro · producto (a S34) · **arquitectura (DA-1 a 85)** · bitacora (a S34) · **manifiesto_narrativo v3.1** · **manifiesto_pois v1.0** (Detectado ≠ Visible ≠ Narrable, A-D) · manifiesto_care_strip · prompt_maestro **v3.7** · dt42 · dt45/dt47 · registro_s24 · restauracion_poi_js
...
- BUG-060 (cerrado): TextExtracts recorta `exchars`>1200 en silencio → truncado cliente `EXTRACT_MAX_CHARS=2500`; `exintro` no cruza encabezados → DT-66
- BUG-062 (cerrado S34): `_finish(source)` pasa motivo a `onEnd(source)`; `visited` se excluye si `source==='visibility-recovery'`
- BUG-061 (cerrado S34): `detectPOI()` ahora chequea `!closestPOI.visited` antes de `activatePOI()`. `activateFromBar()` SIN chequeo a propósito (re-escuchar manual = feature)
...
- **v3.7 (VALIDADA 16/16 S32):** scratchpad en grounding wiki (verificación + `---` + capítulo; `sanitizeNarration()` corta el borrador — strip BUG-059 NO quitar). `MAX_TOKENS=550` (andamiaje). Regla 8 = CIERRE. Anti-regaño. Espejo es/en
...
- **Epílogo (absorbe DT-53):** disparador ÚNICO = cierre confirmado DT-46. Haiku + scratchpad, insumo = capítulos (DT-68), bookend con tesis (único lugar literal), userName ok, sin cache, 0 capítulos → despedida simple
...
v0.9 — **Sesión 34 (18-jul-2026).** Mañana: BUG-062/061 cerrados (sw v49) + README corregido. Tarde: **DT-60 CERRADA, validada en campo ambos caminos** (dataPromise GPS→fetchCityName exportada; barra de compuertas reales 45/90/95%; onPosition sin doble hit Nominatim; sw v50) + **doble matiz DA-77** (genérico silenciado; saludo siempre en explore vía `_flushPendingWelcome()`; sw v51). BUG-052 muere con DT-60. Próximo paso: **tesis de ciudad (commit 2 DA-85, narration.js) en chat nuevo**.
