...
README · REGLAS_IA · docs/: contexto_maestro (alma, pregunta rectora) · producto (DTs activas, visión v2.0 accesibilidad) · arquitectura (DA-1 a DA-79) · bitacora (hasta S27b) · manifiesto_narrativo (voz v3.0) · manifiesto_care_strip · prompt_maestro_follower (oficial, v3.6) · dt42_care_miniprompt · dt45_bienvenida_animada (enmendada S24) · dt47_wizard_mockup_final.html (mockup ratificado) · registro_sesion24_interfaz · restauracion_poi_js (histórico)
...
v0.9 — Sesión 27b completada: **DT-51 (grounding) implementado y en calibración de campo, NO cerrada.** Prueba probabilística n=4 (mismo POI, mismo prompt, 4 navegadores) confirmó autor/fecha en 0/4 — problema estructural, no de redacción. Hipótesis 3 (presupuesto de palabras, `v3.7-test`) probada y descartada — revertida a v3.6 estable. **BUG-050 CERRADO** (sanitización de nombre de ciudad, Nominatim devolvía "Cali ciudad"). sw.js v33. POI_CACHE_VERSION v4. Prompt Maestro v3.6.
...
- **DT-51 (grounding, Sesión 27+27b) — implementado, NO cerrada:** cuatro enfoques de prompt consecutivos (v3.2, v3.3, v3.4, v3.7-test) fallaron en el mismo punto — autor/fecha 0/n. **Recomendación para próxima sesión: enfoque estructural (verificación programática post-generación + regeneración/inserción controlada) — ya no se recomienda seguir ajustando el texto del prompt para este punto específico.** Detalle completo en bitacora.md S27/S27b
