...
## 19. Deuda Técnica Activa *(actualizada a Sesión 36c — 29 julio 2026)*
...
| DT-69 | Guarda por coordenadas (BUG-068, S36c — Opción C, ratificada tras Opción A): comparar las coordenadas del artículo Wikipedia (`prop=extracts\|coordinates`) contra el GPS del caminante; si la distancia excede un umbral (ej. >50 km), descartar el extracto antes de mandarlo a Haiku. No corrige el título — solo detecta un extracto de ciudad equivocado por cualquier vía no anticipada por el hint de OSM (v5). Red de seguridad, no reemplaza a A | Media |
| DT-70 | Limpieza de `retestCityWelcome()` en debug.js (hallazgo S36c, durante BUG-070): referencias muertas de antes del rediseño de tab S35 — `document.getElementById('welcomeBlock')`, `sel.classList.remove('welcome-expandido')` (clase que ya no existe en ningún CSS), `AppState._cityWelcomeCollapsed` (sin lectores en el resto del código). No-ops hoy, pero el botón de debug no resetea el sheet visualmente como su comentario promete | Baja |
| DT-71 | Retiro de `_CITY_NEGATIONS` (narration.js) — condicionado a validar en campo BUG-068 v5. La tabla de negación explícita ("NO es X") queda como red secundaria desde v5 (ya no es la defensa primaria: el extracto llega correcto desde el origen vía hint OSM). No se retira a ciegas en el mismo commit de v5 — este ticket es el que autoriza el retiro una vez confirmado en campo que v5 sostiene el caso sin ella | Baja |
