...
| **BUG-068** | *(Sesión 36, reabierto S36b)* **CAUSA RAÍZ CONFIRMADA, FIX DEFINITIVO PENDIENTE.** 4 versiones de prompt probadas en campo (v1→v4), todas alucinan Palmira Siria. El sesgo de entrenamiento de Haiku sobre "Palmira = ruinas de Siria" supera cualquier instrucción de prompt cuando el nombre de ciudad es solo "Palmira" sin desambiguación. Fix definitivo: usar el título canónico real del artículo de Wikipedia como `cityLabel` en el user prompt ("Palmira (Colombia)" o "Palmira, Valle del Cauca") — construido desde la URL del artículo, no desde `page.title` (que en es.wikipedia devuelve "Palmira" sin paréntesis). `THESIS_PROMPT_VERSION` = v4 en producción (sw v66). | Alta |
