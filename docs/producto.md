...
| BUG-061 | Re-narración de POI ya visitado al salir del modo caminata por tap | Observado 2/2 en Edge y Chrome (S32, tanda Maceta): `visited=true` al completar → `modo caminata OFF — tap del usuario` → 2-4s después `trigger()` re-dispara el mismo POI (cache hit, capítulo #2 guardado y hablado). Correlación perfecta con el tap de salida de walkmode. Hipótesis sin confirmar en código: el tap de salida re-entrega el POI activo o el chequeo post-tap ignora `visited`. Confirmar leyendo `walkmode.js`/`app.js` antes de tocar nada. Si Jaime tocó el marcador a propósito, reclasificar como feature | Alta |
| BUG-062 | La recuperación por `visibilitychange` marca `visited=true` sobre una narración interrumpida — el capítulo se pierde para siempre | Es el "fix 1b" propuesto al cierre de S31, hoy con **tres ocurrencias documentadas en S32** (Firefox tanda-2 Maceta, Safari Sagrada Família, Firefox histórico en el mismo log): el usuario saca el teléfono / cambia de pestaña a mitad de narración → síntesis muere → recovery cierra por `_finish` → `visited=true` → si sigue cerca, el capítulo jamás se re-dispara. Fix propuesto (pendiente de ratificar): en el camino de cierre por visibility-recovery NO marcar `visited` — con cache, el re-disparo es instantáneo y gratis. Escenario exacto de caminata real: foto/WhatsApp a mitad de capítulo | Alta |
