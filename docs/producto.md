...
| DT-77 | **Los POIs de Wikipedia no tienen tipo, y el ícono no informa (S40, evidencia de campo).** `poi.js:346` asigna `icon: '🏛️'` a todo POI que llega por Wikipedia GeoSearch, con el comentario honesto *"icono genérico — Wikipedia no tiene tipo OSM"*. Como Wikipedia es la **fuente primaria** y Overpass la complementaria, la mayoría de los pines del mapa lleva el mismo emoji: en la captura de Palmira (S40) son cuatro POIs y cuatro 🏛️ idénticos — Estatua Simón Bolívar, Chamán Malagán, Catedral. La tabla de doce tipos de `poi.js:76-87` solo alcanza a la minoría de Overpass. Consecuencia: el ícono es textura, no señal, y no diferencia nada al caminante. Señales disponibles sin llamadas extra para derivar tipo: la categoría del artículo, o el tag `wikipedia` de OSM que DA-87 ya cruza para el título canónico. **Toca `POI_CACHE_VERSION`** (cambia la normalización del POI). Interactúa con DT-75: si el clasificador temático se implementa, puede entregar el tipo de paso. Decide también si el emoji tiene lugar en una identidad de DM Serif + paleta noche, o si el pin debe quedar liso | Media |
| DT-78 | **Tabla de emojis duplicada y ya divergente (S40).** El mapa de tipo→emoji existe dos veces: `poi.js:76-87` (doce entradas, dentro del normalizador de POIs) y `app.js:193-198` como `OSM_ICONS` (catorce — añade `park: '🌳'` y `default: '📍'`, que la primera no tiene). Dos fuentes de verdad para lo mismo, ya desincronizadas: el mismo POI puede recibir un emoji en el pin del mapa y otro en la lista del sheet. Es la regla de "funciones únicas — nunca duplicar" aplicada a una tabla. Consolidar en una sola exportada, probablemente desde `poi.js`. Se resuelve junto con DT-77 o queda sin sentido | Baja |
...
| ID | Descripción | Causa | Prioridad |
|----|-------------|-------|-----------|
| BUG-071 | **Las etiquetas de POI colisionan entre sí y se salen de la pantalla.** Evidencia de campo (Palmira, sw v74, capturas 1:16 y 1:17): la etiqueta roja del POI activo —*"…Señora del Rosario del…"*— queda recortada por el borde izquierdo de la pantalla y montada encima de la etiqueta *"Chamán Malagán"*. Ambas ilegibles | El `divIcon` del marcador mide 80×60 px (`gps.js:627-628`) pero `.poi-pin-label` lleva `white-space: nowrap` (`explore.css:173`), así que la etiqueta se desborda del icono sin límite y Leaflet no reposiciona nada. Con nombres de Wikipedia no es ocasional sino sistemático: son largos por naturaleza (*"Catedral de Nuestra Señora del Rosario del Palmar"* no cabe en ningún ancho razonable). **Nota:** ocultar el nombre en los POIs no activos elimina la colisión entre etiquetas, pero **no** resuelve el recorte — en la captura el POI activo es justamente el del nombre más largo. Hace falta además truncar, o anclar la etiqueta al viewport en vez de al pin | Alta |
