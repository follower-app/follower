...
| ~~BUG-068~~ | *(Sesión 36, narration.js)* **CAUSA CONFIRMADA + FIX APLICADO.** Palmira, Colombia generó tesis y prólogo sobre Palmira/Palmyra de Siria ("La gloria que el desierto recuerda", oasis, ruinas, reina). El `THESIS_SYSTEM_PROMPT` no tenía instrucción de anclarse exclusivamente al extracto dado e ignorar conocimiento previo de ciudades homónimas. Fix: sección "PROHIBIDO — OTRA CIUDAD CON EL MISMO NOMBRE" + línea "Pertenencia:" en el scratchpad + `THESIS_PROMPT_VERSION` v1 → v2 (invalida caches con alucinación automáticamente). | Alta |
