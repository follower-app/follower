...
const Narration = (() => {
  ...
  const CONFIG = {
    ...
    PROMPT_VERSION: 'v3.9',  // BUG-071 (S43): la regla 1 ofrecia tres ejemplos de apertura y uno de ellos —"Mira hacia..."— fusionaba las reglas 1 y 2 en una sola frase, asi que ganaba siempre bajo el presupuesto de 90-130 palabras. Los tres ejemplos se retiran: un menu mas grande habria repetido el patron con otra mascara, porque los tres eran deicticos. La identificacion deja de estar obligada a ser la primera frase (pasa a "dentro de las dos primeras") y se prohibe la deixis lateral: buildPrompt() no manda ningun dato espacial, y aunque lo mandara, el texto se genera al arrancar y suena durante 60-67m de caminata — cualquier direccion llega tarde. Mismo cambio en las dos variantes de idioma. v3.8 (S39): DT-68 — el scratchpad declara "Faceta: <3-5 palabras>" (DA-85 §3 enmienda S38 §5) y el registro cacheado pasa de texto plano a {text, faceta} (§6). El ++ es obligatorio por ambos motivos y una sola invalidacion paga los dos cambios de formato. v3.7 (S32): scratchpad deliberado en grounding wiki (cara buena de BUG-059 convertida en tecnica: chain-of-thought escrito, cortado por sanitizeNarration) + presupuesto de longitud en el scratchpad + regla 8 CIERRE (sin promesa hacia adelante) + regla anti-regano en LIMITES ESTRICTOS. DT-62 CERRADA: canal system verificado punta a punta (cliente + Worker passthrough, prueba directa)
...
1. IDENTIFICACIÓN — El caminante debe poder identificar el lugar dentro de las dos primeras frases. No tiene que ser la primera: el capítulo puede abrir por el rasgo que salta a la vista, por un hecho, por una fecha o por la pregunta que el lugar provoca, y ubicar después. Nunca uses direcciones relativas al cuerpo del caminante — ni izquierda, ni derecha, ni al frente, ni detrás. Dentro del capítulo cualquier dirección llega tarde: el texto se genera al principio y suena mientras el caminante sigue avanzando. Ubicar por lo que se reconoce ("no hay forma de confundirlo", "es el único de la cuadra con...") funciona desde cualquier ángulo. La referencia vertical es legítima, pero no debe ser el recurso de siempre.
...
1. IDENTIFICATION — The walker must be able to identify the place within the first two sentences. It does not have to be the first one: the chapter may open with the feature that stands out, with a fact, with a date, or with the question the place provokes, and place it afterwards. Never use directions relative to the walker's body — no left, no right, no ahead, no behind. Inside the chapter any direction arrives late: the text is generated at the start and plays while the walker keeps moving. Placing it by what can be recognised ("there is no mistaking it", "it is the only one on the block with...") works from any angle. A vertical reference is legitimate, but it must not be the default move.
